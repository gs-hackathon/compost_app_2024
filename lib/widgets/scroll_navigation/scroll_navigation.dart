...
class ScrollNavigation extends StatefulWidget {
  ...
  const ScrollNavigation({
    ...
    required this.pages,
    required this.items,
    required this.pagesActionButtons,
    this.initialPage = 0,
    this.showIdentifier = true,
    this.physics = true,
    this.maxWillPopLocations = 5,
    required NavigationBarStyle barStyle,
    required NavigationBodyStyle bodyStyle,
    required NavigationIdentiferStyle identiferStyle,
  })  : assert(items.length == pages.length),
        barStyle = barStyle ?? const NavigationBarStyle(),
        bodyStyle = bodyStyle ?? const NavigationBodyStyle(),
        identiferStyle = identiferStyle ?? const NavigationIdentiferStyle(),
        super(key: key);
...
class ScrollNavigationState extends State<ScrollNavigation> {
  ...
  int _maxItemsCache = 0;
  int _currentIndex = 0;
  bool _itemTapped = false;
  double _navSize = 0.0;
  double _identifierLength = 0.0;

  final List<int> _popUpCache = [];
  final List<Widget> _navIcons = [];
  final List<Widget> _navTexts = [];
  final List<Widget> _pagesActionButtons = [];

  bool _verticalPosition = false;
  Orientation _orientation = Orientation.portrait;
  PageController _controller = PageController();
  NavigationBarStyle _barStyle = const NavigationBarStyle();
  NavigationBodyStyle _bodyStyle = const NavigationBodyStyle();
  NavigationIdentiferStyle _identifierStyle = const NavigationIdentiferStyle();
  ...
  void initState() {
    ...
    for (int i = 0; i < widget.items.length; i++) {
      ...
      _navTexts.add(item.title.isNotEmpty
          ? Text(
              item.title,
